...
const flowVec = (direction?: FerrofluidProps["flowDirection"]): [number, number] => {
  switch (direction) {
    case "up": return [0, 1];
...
const callIfFunction = (value: unknown, method: string) => {
  const candidate = value as Record<string, unknown> | null;
  const callback = candidate?.[method];
  if (typeof callback === "function") callback.call(value);
};
  ...
  useEffect(() => {
    ...
    try {
      ...
      const loop = (time: number) => {
        ...
        if (!paused && !reducedMotion) {
          try {
            ...
            console.warn("Ferrofluid render stopped after a WebGL error.", error);
      ...
      return () => {
        ...
        callIfFunction(programRef.current, "remove"); callIfFunction(geometryRef.current, "remove");
        callIfFunction(meshRef.current, "remove"); callIfFunction(rendererRef.current, "destroy");
        programRef.current = null; geometryRef.current = null; meshRef.current = null; rendererRef.current = null;
      };
    } catch (error) {
      console.warn("Ferrofluid background is unavailable; using fallback background.", error);
      callIfFunction(rendererRef.current, "destroy");
      rendererRef.current = null;
      return undefined;
    }
  }, [colors, dpr, fluidity, flowDirection, glow, mouseDampening, mouseInteraction, mouseRadius, mouseStrength, opacity, paused, rimWidth, scale, sharpness, shimmer, speed, turbulence]);

  return <div aria-hidden="true" className={`${styles.container} ${className ?? ""}`} ref={containerRef} style={mixBlendMode ? { mixBlendMode } : undefined} />;
}
