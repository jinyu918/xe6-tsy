...
| `REALTIME_TTS_DOWNLINK` | `none` | `none` = subtitles only (forces mock TTS); `pcm` = whole-clip `pcm_s16le/24kHz/mono` over DataChannel; `opus` = 120ms-buffered, 20ms-paced `Opus/48kHz/mono` WebRTC at 32kbps |
